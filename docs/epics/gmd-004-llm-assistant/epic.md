...
| S2/R1-S1 | `packages/contracts/src/index.ts` and `packages/plugin-sdk/src/index.ts` | shared contract / capability boundary | Defines the bounded model-session policy and rejects undeclared handler or capability access before a bundle reaches the service. |
...
| S2/R1-S1 | `packages/contracts/src/index.test.ts`, `packages/plugin-sdk/src/index.test.ts`, `plugins/assistant/src/index.test.ts`, `apps/server/tests/plugins/plugin_runtime_service.test.ts`, and `apps/server/tests/http/authentication.test.ts` | Bounded policies reject unknown/duplicate tools, host registration requires the declared Context capabilities, the static bundle sends its prompt/tool list, and the production host lists and dispatches the active contribution. | focused automated passing |
...
| S2/R4-S1, S2/R4-S3 | `apps/web/src/AssistantContext.test.tsx` and `apps/web/src/App.test.tsx` | The connected composer retains a prompt while busy, prevents duplicate submit, renders returned service sources, and directs disconnected owners to Settings. | focused automated passing |
| S2/R4-S1, S2/R4-S2 | `apps/web/src/App.test.tsx` and `apps/web/src/App.stories.tsx` | Active descriptor contributions mount and disappear with plugin lifecycle; the production Storybook interaction suite covers the corresponding workbench shell. | focused automated passing |
